...
Ass seen from the above table, there is a notable difference in the amount of samples in each of the categories. This could have a big influence on the training data, as the model would be prone to overfit on the classes with more samples. To account and test for this imbalance, the model can be run in a script that balances the data (by calculating class weights, thereby letting the prominent classes weigh less in the model) and one that does not. See **Usage** and **Results** for more on this issue.
